...
class MVIDataset3D(torch.utils.data.Dataset):
    def __init__(self,path,train=True,augment=True):
        ...
        self.train=train
        self.aug=augment
        images=sorted(glob.glob(path+"/*_T2.nii"))
        labels=sorted(glob.glob(path+"/*_T2.nrrd"))
...
if __name__=='__main__':
    dataset=MVIDataset3D('/newdata/why/MVI_Formatted',train=True)
