...
class PromiseDataset3D(torch.utils.data.Dataset):
    def __init__(self,path,train=True,augment=True):
        ...
        self.train=train
        self.aug=augment
        images=sorted(glob.glob(path+"/Case??.mhd"))
        labels=sorted(glob.glob(path+"/*_segmentation.mhd"))
    ...
    def reset_spacing(self, itkimage,resamplemethod=sitk.sitkBSpline,newSpacing=[0.5,0.5,0.75]):
...
if __name__=='__main__':
    dataset=PromiseDataset3D('/newdata/why/PROMISE12',train=True)
