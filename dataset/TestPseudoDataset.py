...
class TestPseudoDataset(torch.utils.data.Dataset):
    def __init__(self,path,mode='train',augment=True):
        ...
        self.mode=mode
        self.aug=augment
        images=sorted(glob.glob(os.path.join(path, "*/*_img.npy")))
        labels=sorted(glob.glob(os.path.join(path, "*/*_segpseudo.npy")))
        inputs=sorted(glob.glob(os.path.join(path, "*/*_input.npy")))
        ...
        for i in range(len(images)):
            ...
            label_sr=np.load(images[i])
            label_seg=np.load(labels[i])
            # lesion_arr[lesion_arr>1]=1  # 只做WT分割\
            img=np.load(inputs[i])
        ...
        #     inputs=inputs[n_train:n_train+n_val]
        #     for i in range(len(images)):
        ...
        #     inputs=inputs[n_train+n_val:n_train+n_val+n_test]
        #     # val2
    ...
    #     return ndimage.interpolation.zoom(img,[0.5,0.5,0.5],order=1),mask,img
...
class TestPseudoLiverDataset(torch.utils.data.Dataset):
    def __init__(self,path,mode='train',augment=True):
        self.data=[]
        self.label_seg=[]
        self.label_sr=[]
...
if __name__=='__main__':
    dataset=TestPseudoDataset('/newdata/why/BraTS20')
