...
model2 = PointUnet(4, 1,device=device).to(device)
...
def TestModel():
    ...
    for whatever, data in enumerate(test_dataset1):
        ...
        #             if output_list[i, j, k] > 0.49:
        #                 pointSet.append([i, j, k, image[i, j, k]])
        #             else:
        #                 if random.random() < output_list[i, j, k]:
        #                     pointSet.append([i, j, k, image[i, j, k]])

        # pointSet=np.array(pointSet)
        # inputs3D = pt.autograd.Variable(pt.from_numpy(pointSet)).type(pt.FloatTensor).unsqueeze(0).to(device)
        # with pt.no_grad():
